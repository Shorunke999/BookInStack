...
;(function (global, factory) {
  ...
  const DEFAULT_CONFIG = {
    baseUrl:   'https://bookinstack-main-wuhnit.free.laravel.cloud/api',
    version:   '1.1.0',
    timeout:   30000,
    returnUrl: null,
    onSuccess: null,
  ...
  const widget = {
    ...
    if (_enableNegotiate && mode.mode !== 'ticket') {
      ...
      const refreshNights = () => {
        ...
        if (badge) badge.textContent = `${_reservationUnit == 'day' ? '☀️' : '🌙'}  ${n} ${_reservationUnit} ${n !== 1 ? 's' : ''}`;
        refreshPriceDisplay();
      };
      ...
      const selectCat = (el) => {
        ...
        if (submitBtn) submitBtn.style.display  = !isFixed  ? 'none' : 'block';
    ...
    async init(options = {}) {
      ...
      try {
        ...
        _bookingOpen    = status.open !== false;
         _reservationUnit = status.reservation_unit;
